...
input color  In_ButtonColorOn      = clrDodgerBlue;       // Cor do botão ativo
input color  In_ButtonColorOff     = clrSlateGray;        // Cor dos botões inativos
input int    In_ButtonCorner       = CORNER_LEFT_UPPER;   // Canto da tela onde os botões ficam
input int    In_ButtonX            = 20;                  // Distância X do canto
input int    In_ButtonY            = 40;                  // Distância Y do canto
input int    In_ButtonWidth        = 110;                 // Largura do botão
input int    In_ButtonHeight       = 22;                  // Altura do botão
...
double BufferMin[8];
double BufferMax[8];
bool   BufferHasValue[8];
string ButtonNames[4] = {"CSV_PLOT_BTN_0","CSV_PLOT_BTN_1","CSV_PLOT_BTN_2","CSV_PLOT_BTN_3"};
int    ActiveButton = 0;
bool   ButtonsCreated = false;
bool   ButtonLabelsReady = false;
string IndicatorTitle = "CSV Reader Plot v3.1";
...
void SetBufValue(const int idx, const int bar, const double value){
   ...
   if(value!=EMPTY_VALUE && idx>=0 && idx<8)
   {
      if(value < BufferMin[idx]) BufferMin[idx] = value;
      if(value > BufferMax[idx]) BufferMax[idx] = value;
      BufferHasValue[idx] = true;
   }
}

string PlotNameForButton(const int idx)
{
   if(idx < ArraySize(PlotNames) && PlotNames[idx] != "")
      return PlotNames[idx];
   return "Series" + (string)(idx+1);
}
...
void ResetBufferStats()
{
   for(int i=0;i<8;i++)
   {
      BufferMin[i] = DBL_MAX;
      BufferMax[i] = -DBL_MAX;
      BufferHasValue[i] = false;
   }
}

void UpdatePlotVisibility()
{
   int plots = MathMin(UsedPlots, 8);
   for(int i=0;i<plots;i++)
   {
      bool active = (i==ActiveButton);
      PlotIndexSetInteger(i, PLOT_DRAW_TYPE, active?DRAW_LINE:DRAW_NONE);
      PlotIndexSetInteger(i, PLOT_SHOW_DATA, active);
   }
   ApplyActiveScale();
   ChartRedraw();
}

void ApplyActiveScale()
{
   int wnd = ChartWindowFind(0, IndicatorTitle);
   if(wnd<0)
      wnd = 1; // assume first subwindow

   if(ActiveButton<0 || ActiveButton>=8 || !BufferHasValue[ActiveButton])
   {
      ChartSetInteger(0, CHART_WINDOW_SCALEFIX, wnd, false);
      ChartSetDouble(0, CHART_WINDOW_FIXED_MIN, wnd, 0.0);
      ChartSetDouble(0, CHART_WINDOW_FIXED_MAX, wnd, 0.0);
      return;
   }
   double mn = BufferMin[ActiveButton];
   double mx = BufferMax[ActiveButton];
   if(!MathIsValidNumber(mn) || !MathIsValidNumber(mx) || mn==DBL_MAX || mx==-DBL_MAX)
   {
      ChartSetInteger(0, CHART_WINDOW_SCALEFIX, wnd, false);
      return;
   }
   ...
   if(mx==mn)
   {
      double delta = (MathAbs(mx) > 0.0001)? MathAbs(mx)*0.01 : 0.01;
      mn -= delta;
      mx += delta;
   }
   ChartSetInteger(0, CHART_WINDOW_SCALEFIX, wnd, true);
   ChartSetDouble(0, CHART_WINDOW_FIXED_MIN, wnd, mn);
   ChartSetDouble(0, CHART_WINDOW_FIXED_MAX, wnd, mx);
}

void UpdateButtonLabels()
{
   int plots = MathMin(UsedPlots, 4);
   for(int i=0;i<plots;i++)
   {
      string text = PlotNameForButton(i);
      ObjectSetString(0, ButtonNames[i], OBJPROP_TEXT, text);
   }
   ButtonLabelsReady = true;
}
...
void UpdateButtonStates()
{
   ...
   for(int i=0;i<plots;i++)
   {
      color bg = (i==ActiveButton)?In_ButtonColorOn:In_ButtonColorOff;
      ObjectSetInteger(0, ButtonNames[i], OBJPROP_BGCOLOR, bg);
      ObjectSetInteger(0, ButtonNames[i], OBJPROP_COLOR, clrWhite);
      ObjectSetInteger(0, ButtonNames[i], OBJPROP_STATE, (i==ActiveButton));
   }
   ApplyActiveScale();
}
...
void EnsureButtons()
{
   ...
   for(int i=0;i<plots;i++)
   {
      ...
      ObjectSetInteger(0, name, OBJPROP_CORNER, In_ButtonCorner);
      ObjectSetInteger(0, name, OBJPROP_XDISTANCE, In_ButtonX);
      ObjectSetInteger(0, name, OBJPROP_YDISTANCE, In_ButtonY + i*(In_ButtonHeight+4));
      ObjectSetInteger(0, name, OBJPROP_XSIZE, In_ButtonWidth);
      ObjectSetInteger(0, name, OBJPROP_YSIZE, In_ButtonHeight);
      ObjectSetInteger(0, name, OBJPROP_COLOR, clrWhite);
      ObjectSetInteger(0, name, OBJPROP_BGCOLOR, In_ButtonColorOff);
      ObjectSetInteger(0, name, OBJPROP_BORDER_COLOR, clrBlack);
      ObjectSetString(0, name, OBJPROP_FONT, "Arial");
      ObjectSetInteger(0, name, OBJPROP_FONTSIZE, 9);
      ObjectSetString(0, name, OBJPROP_TEXT, "Series" + (string)(i+1));
   }

   ButtonsCreated = true;
   ButtonLabelsReady = false;
   UpdateButtonStates();
   ApplyActiveScale();
}
...
int OnInit()
{
   IndicatorSetString(INDICATOR_SHORTNAME, IndicatorTitle);
   SetupPlots();
                  ...
                  const string &sparam)
{
   ...
   {
      for(int i=0;i<MathMin(UsedPlots,4);i++)
      {
